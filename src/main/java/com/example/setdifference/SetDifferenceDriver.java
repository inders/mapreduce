 ...
 * Highlights ->
 * Mapper uses FileSplit from context to get FileName a split belongs to
 * setA, setB file names are passed to map and reduce using configuration
 * custom properties
 *
 * Approach
 * 1. each split will be run by a different mapper
 * 2. Mapper Output -> key = line/word, value = fileName
 * 3. Reducer Check -> if a key contains setB has value don't emit in final
 * output
 */
